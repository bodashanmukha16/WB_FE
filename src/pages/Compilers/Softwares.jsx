...
export default function Softwares() {
  const [updatedResult, setUpdated] = useState(softwares)
  ...
  const newResult = (e)=>{
    ...
    if (value.trim() === "") {
    setUpdated(softwares)
    return;
  }
    const items1 = softwares.filter(item=>
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="w-full max-w-sm min-w-[200px] mb-4">
            <input
              ...
              placeholder="Search Your software" onChange={newResult} value={updatedValue}
            />
          </div>
            ...
            {updatedResult.map((software) => (
              ...
              >
                <div className="w-16 h-16 bg-gradient-to-br from-blue-500 to-purple-600 rounded-xl flex items-center justify-center mb-6">
                  <i className={`fas ${software.icon} text-white text-2xl`}></i>
                </div>

                <h3 className="text-2xl font-bold mb-3">{software.name}</h3>
                ...
                <a
                  ...
                  className={`inline-block px-6 py-3 bg-gradient-to-r ${software.color} text-white rounded-lg font-semibold hover:shadow-lg transition-all duration-300`}
                >
                  Open software
                </a>
              </div>
